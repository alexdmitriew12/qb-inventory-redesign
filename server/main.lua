...
local function SaveOwnedGloveboxItems(plate, items)
	if Gloveboxes[plate].label == "Schowek-None" or not items then return end
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			if name == "stash" then
				...
				secondInv.label = "Schowek-"..id
				secondInv.maxweight = maxweight
				secondInv.inventory = {}
				secondInv.slots = slots
				if Stashes[id] and Stashes[id].isOpen then
					secondInv.name = "none-inv"
					secondInv.label = "Schowek-None"
		...
		TriggerClientEvent('QBCore:Notify', src, 'Nie możesz tego zrobić', 'error')
	end
end)
